...
- Terms of acceptance`,
    ...
    const sectionPrompt =
      ...
      `Generate professional content for the "${sectionTitle}" section of this Statement of Work.`;
  ...
  async generateSection(options: AIGenerationOptions): Promise<string> {
    ...
    for (let attempt = 1; attempt <= 2; attempt++) {
      try {
        ...
        if (attempt === 2) {
          throw new Error(
            "All AI generation attempts failed. Please check your API keys and try again.",
          );
        }
      }
    }
